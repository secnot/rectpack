...
class BinFactory(object):
    ...
    def __init__(self, width, height, count, pack_algo, *args, **kwargs):
        ...
        Parameters:
            ...
            count:
            pack_algo
            bid_prefix (str): Optional string that will be used as a prefix for
                the random identifiers generated for each bin
        """
        ...
        self._prefix = kwargs.pop('bid_prefix', '') + '-'
        
        # Initialize reference bin used to calculate fitness
        self._ref_bin = self._create_bin()
       
    def _create_bin(self):
        self._algo_kwargs['bid'] = self._prefix + str(uuid.uuid4())
    ...
    def fitness(self, width, height):
        """Return fitness for placing rectangle with dimmensions width*height in one
        empty bin of the type generated by the factory"""
        return self._ref_bin.fitness(width, height)

    def fits_inside(self, width, height):
        """Returns True if a rectangle of dimmensions width*height fit into one of
        the bitns of the type generated by the factory"""
...
class PackerBNFMixin(object):
    ...
    def add_rect(self,
            width: Number,
            height: Number, 
            rid: Optional[Union[str, int]] = None
        ...
        while True:
            ...
            if rect is not None:
                return self._open_bins[0].bid, rect
...
class PackerBBFMixin(object):
    ...
    def add_rect(self,
        ...
        # Try packing into one of the open bins
        ...
        try:
            ...
            rect = best_bin.add_rect(width, height, rid)
            return best_bin.bid, rect
        except ValueError:
            pass    

        # Didn't fit into an  open bin try open more
        while True:
            # can we find an unopened bin that will hold this rect?
            new_bin = self._new_open_bin(width, height, rid=rid)
            if new_bin is None:
            ...
            # _new_open_bin may return a bin that's too small,
            # so we have to double-check
            rect = new_bin.add_rect(width, height, rid)
            if rect is not None:
                return new_bin.bid, rect
...
class PackerOnline(object):
    ...
    def pop_closed(self) -> PackingAlgorithm:
        """Pop oldest closed bin
        
        Raises:
            IndexError: No closed bins available
        """
        return self._closed_bins.pop()
...
class Packer(PackerOnline):
    ...
    def add_bin(self, width, height, count=1, **kwargs):
        ...
        Parameters:
            width: Bin width
            height: Bin height
            count: Number of bins of the given dimmensions to add
            bid (Option):
        """
